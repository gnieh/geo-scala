...
object polyline {
  ...
  /** Encode a [[LineString]] into its polyline representation.
    * @param line
    *   the linestring to encode
    * @return
    *   the polyline string
    */
  def encode(line: LineString): String = {
    line.coordinates
      .scanLeft(Coordinate(0, 0) -> Coordinate(0, 0)) { case ((old, _), curr) =>
  ...
  /** Attempt to decode a polyline string - fails if the input is not a valid polyline
    ...
    *   the polyline string
    * @return
    *   the decoded [[LineString]] or a describing `Failure`
    */
      ...
      Success(
        LineString(
          chunks
            ...
            .toList
            .scanRight(Coordinate(0.0, 0.0)) { case (old, delta) =>
  ...
  private def poly2Num(pieces: List[Int]): Double = {
    val num = pieces.reverse.zipWithIndex.foldLeft(0) { case (acc, (piece, idx)) =>
      acc | (piece << (idx * 5))
    }
