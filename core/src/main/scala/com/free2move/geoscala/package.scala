...
package object geoscala {
  ...
  /**
    ...
    * @tparam Properties type of representing the `properties` of a [[Feature]], `Nothing` in case it's not a feature or feature collection.
    */
  sealed trait GeoJson[Properties]

  /**
    * A feature collection as of §3.3 of RFC 7946.
    * @param features the features of this collection, potentially empty
    * @tparam Properties type of representing the `properties` of a [[Feature]], `Nothing` in case it's not a feature or feature collection.
    */
  ...
  /**
    ...
    * @param properties the properties object attached to this feature
    * @param geometry the [[Geometry]] attached to this feature
    * @tparam Properties type of representing the `properties` of a [[Feature]], `Nothing` in case it's not a feature or feature collection.
    */
  ...
  /**
    ...
    * @param coordinates a single coordinate to specify the point's location.
    */
  case class Point(coordinates: Coordinate) extends Geometry

  /**
    * A collection of points - §3.1.3 of RFC 7946.
    * @param coordinates a `List` of coordinates representing the single points.
    */
  ...
  /**
    ...
    * @param coordinates the list of coordinates representing the points this line string goes through.
    */
  case class LineString(coordinates: List[Coordinate]) extends Geometry

  /**
    * The multi version of a line string - §3.1.5 of RFC 7946.
    * @param coordinates list of line string data, which itself is a list of coordinates
    */
  case class MultiLineString(coordinates: List[List[Coordinate]]) extends Geometry

  /**
    * A polygon is a 2d shape which can have holes - §3.1.6 of RFC 7946. The first list of coordinates is the outer ring,
    * the following coordinate list are the holes.
    * @param coordinates list of outer ring followed by (optional) holes.
    */
  case class Polygon(coordinates: List[List[Coordinate]]) extends Geometry

  /**
    * The multi version of a polygon - §3.1.7 of RFC 7946.
    * @param coordinates the list of polygon data
    */
